...
module Duvido {
	export class User {
		...
		static fromToken(token : string, callback : (err : Error, user : User) => void) {
			DB.users.findOne({tokens : token}, function(err : Error, user : DB.User) {
				if (user === null) {
					Facebook.fetchMe(token, function(err : Error, userInfo : Facebook.User) {
		...
		getToken(callback : (err : Error, token : string) => void) {
			var id = this.id;
			DB.users.findOne({userId : id}, function(err : Error, user : DB.User) {
				if (err) { callback(err, null); return; }
				if (user && user.tokens) {
		...
		getAvatar(callback : (err : Error, buf : Buffer) => void) {
			var _this = this;
			DB.users.findOne({userId : this.id}, function(err : Error, user : DB.User) {
				if (err) { callback(err, null); return; }
				if (user && user.avatar) {
					...
					Facebook.fetchAvatar(_this.id, function(err : Error, buff : Buffer) {
						if (err) { callback(err, null); return; }
						callback(null, buff);
		...
		getName(token : string, callback : (err : Error, name : string) => void) {
			...
			DB.users.findOne({userId : this.id}, function(err : Error, user : DB.User) {
				...
				if (user && user.name) {
					...
					Facebook.fetchUser(token, _this.id, function(err : Error, userInfo : Facebook.User) {
						if (err) { callback(err, null); return; }
